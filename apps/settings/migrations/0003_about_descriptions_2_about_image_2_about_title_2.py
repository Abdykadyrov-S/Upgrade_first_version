# Generated by Django 4.2.7 on 2023-12-23 17:20

import ckeditor.fields
from django.db import migrations, models
import django_resized.forms


class Migration(migrations.Migration):

    dependencies = [
        ('settings', '0002_alter_settings_options_data_clients_title_and_more'),
    ]

    operations = [
        migrations.AddField(
            model_name='about',
            name='descriptions_2',
            field=ckeditor.fields.RichTextField(blank=True, null=True, verbose_name='Информационный текст №2'),
        ),
        migrations.AddField(
            model_name='about',
            name='image_2',
            field=django_resized.forms.ResizedImageField(blank=True, crop=None, force_format='WEBP', keep_meta=True, null=True, quality='100', scale=None, size=[1920, 1080], upload_to='about_image', verbose_name='Фотография №2'),
        ),
        migrations.AddField(
            model_name='about',
            name='title_2',
            field=models.CharField(default=1, max_length=255, verbose_name='Заголовок №2'),
            preserve_default=False,
        ),
    ]
